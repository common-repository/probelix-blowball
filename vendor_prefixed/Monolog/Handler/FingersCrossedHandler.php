...
namespace PbxVendor\Monolog\Handler;

use PbxVendor\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use PbxVendor\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
use PbxVendor\Monolog\Logger;
use PbxVendor\Monolog\ResettableInterface;
use PbxVendor\Monolog\Formatter\FormatterInterface;
use PbxVendor\Psr\Log\LogLevel;
...
class FingersCrossedHandler extends Handler implements ProcessableHandlerInterface, ResettableInterface, FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = true, bool $stopBuffering = true, $passthruLevel = null)
    {
        ...
        if (!$this->handler instanceof HandlerInterface && !is_callable($this->handler)) {
            throw new \RuntimeException("The given handler (".json_encode($this->handler).") is not a callable nor a PbxVendor\Monolog\Handler\HandlerInterface object");
        }
    }
